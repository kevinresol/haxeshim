...
class WindowsExe {
  ...
  static public function ensureVersionHaxelib(versionPath:String):String {
    ...
    if (bundled.exists()) {
      // #region agent log
      agentLog('H2', 'WindowsExe.hx:ensureVersionHaxelib', 'already shimmed', {
    ...
    if (normal.stat().size < 10000) {
      // #region agent log
      agentLog('H2', 'WindowsExe.hx:ensureVersionHaxelib', 'haxelib.exe already looks like a shim', {
        versionPath: versionPath,
        size: normal.stat().size,
      });
      // #endregion
      return normal;
    }
    ...
    if (!shimJs.exists()) {
      // #region agent log
      agentLog('H2', 'WindowsExe.hx:ensureVersionHaxelib', 'haxelibshim.js missing; skip wrap', {
        shimJs: shimJs,
      });
      // #endregion
      return normal;
    ...
    try {
      ...
      // #region agent log
      agentLog('H2', 'WindowsExe.hx:ensureVersionHaxelib', 'wrapped version haxelib.exe', {
        versionPath: versionPath,
        bundled: bundled,
        shimJs: shimJs,
      });
      ...
      // #region agent log
      agentLog('H2', 'WindowsExe.hx:ensureVersionHaxelib', 'wrap failed', {
        error: Std.string(e),
        versionPath: versionPath,
      });
      // #endregion
  ...
  // #region agent log
  static function agentLog(hypothesisId:String, location:String, message:String, data:Dynamic)
    try {
